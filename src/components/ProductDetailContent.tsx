    ...
    <div className="pt-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <nav className="flex items-center gap-2 text-xs text-gray-600 mb-8">
          ...
          <span>/</span>
          <Link href="/#products" className="hover:text-[var(--accent)] transition-colors">{shoe.categoryPersian}</Link>
          <span>/</span>
          <span className="text-gray-400">{shoe.namePersian}</span>
        </nav>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[shoe.image, ...shoe.images.slice(0, 2)].map((img, i) => (
                <button key={i} onClick={() => setActiveImage(i)}
                  className={`aspect-square bg-[var(--muted)] overflow-hidden border-2 transition-all ${
          ...
          <div className="flex flex-col">
            <p className="text-[10px] uppercase tracking-widest text-gray-500 mb-2">{shoe.brand}</p>
            <h1 className="text-2xl font-bold text-white mb-1">{shoe.namePersian}</h1>
            <p className="text-sm text-gray-600 mb-4">{shoe.name}</p>

            <div className="flex items-center gap-2 mb-6">
              <div className="flex items-center">
                {[...Array(5)].map((_, i) => (
                  <svg key={i} className={`w-3.5 h-3.5 ${i < Math.round(shoe.rating) ? "text-[var(--accent)]" : "text-gray-700"}`} fill="currentColor" viewBox="0 0 20 20">
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                  </svg>
                ))}
              </div>
              <span className="text-xs text-gray-500">{shoe.rating} / 5</span>
            </div>
            ...
            <div className="mb-6">
              {shoe.sale && shoe.discount ? (
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[10px] bg-red-600/20 text-red-500 px-2 py-0.5 font-medium border border-red-500/30">
                    -{shoe.discount}%
                  </span>
                </div>
              ) : (
            ...
            <p className="text-sm text-gray-400 leading-relaxed mb-8">{shoe.descriptionPersian}</p>

            <div className="mb-6">
              <h3 className="text-xs font-semibold text-gray-300 mb-3">رنگ:</h3>
            ...
            <div className="mb-8">
              <h3 className="text-xs font-semibold text-gray-300 mb-3">سایز:</h3>
            ...
            <button onClick={() => addItem(shoe, selectedSize || shoe.sizes[0], selectedColor || shoe.colors[0].name)}
              className="w-full py-3.5 btn-primary text-sm font-semibold tracking-wider uppercase mb-4">
              افزودن به سبد خرید
            </button>
            ...
            <div className="grid grid-cols-3 gap-4 py-6 border-t border-[var(--border)]">
              {[
                ...
                <div key={f.label} className="text-center">
                  <svg className="w-5 h-5 mx-auto text-[var(--accent)] mb-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
