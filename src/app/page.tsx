...
function FilterSidebar({
  ...
  return (
    <div className={isDrawer ? "flex flex-col h-full" : "sidebar-sticky pl-4 lg:pl-6"}>
      {isDrawer ? (
        <>
          <div className="filter-drawer-header">
            <div className="flex items-center gap-3">
              ...
              {hasActiveFilters && (
                <span className="text-[10px] bg-[var(--accent)] text-white px-2 py-0.5 rounded-full font-medium">
                  {[activeCategory !== "all" && activeCategory, ...selectedBrands, ...selectedColors, ...selectedSizes, saleOnly && "sale", inStockOnly && "stock", minRating > 0 && "rate", searchQuery && "search"].filter(Boolean).length}
                </span>
              )}
            </div>
            <div className="flex items-center gap-2">
              {hasActiveFilters && (
                <button onClick={clearAll} className="text-[10px] text-[var(--accent)] hover:underline px-2 py-1">
                  حذف همه
                </button>
              )}
              <button onClick={onClose} className="text-gray-500 hover:text-white p-1.5 rounded-lg hover:bg-[var(--muted)] transition-colors">
          ...
          <div className="filter-drawer-body">
            <Section label="دسته‌بندی" id="cat">
              ...
              {categories.map((cat) => (
                <div key={cat.id} className={`filter-tree-item ${activeCategory === cat.id ? "active" : ""}`} onClick={() => setActiveCategory(cat.id)}>
            ...
            <Section label="قیمت" id="price">
              <div className="flex items-center gap-2 mb-2">
                <input type="number" className="price-input" placeholder="از" value={priceRange[0]} onChange={(e) => setPriceRange([Number(e.target.value) || PRICE_MIN, priceRange[1]])} />
                <span className="text-gray-600 text-xs">تا</span>
                <input type="number" className="price-input" placeholder="تا" value={priceRange[1]} onChange={(e) => setPriceRange([priceRange[0], Number(e.target.value) || PRICE_MAX])} />
              </div>
              <input type="range" className="price-slider" min={PRICE_MIN} max={PRICE_MAX} step={50000} value={priceRange[1]} onChange={(e) => setPriceRange([priceRange[0], Number(e.target.value)])} />
            ...
            <Section label="امتیاز" id="rating">
              {[4, 3, 2, 1].map((star) => (
                <div key={star} className="filter-checkbox" onClick={() => setMinRating(minRating === star ? 0 : star)}>
                  <input type="radio" checked={minRating === star} onChange={() => {}} name="rating" />
                  <div className="flex items-center gap-0.5">
                    {[...Array(5)].map((_, i) => (
                      <svg key={i} className={`rating-star ${i < star ? "text-[var(--accent)]" : "text-gray-700"}`} fill="currentColor" viewBox="0 0 20 20">
            ...
            <Section label="رنگ" id="color">
              <div className="flex flex-wrap gap-2">
                {ALL_COLORS.map((c) => (
                  <button key={c.name} onClick={() => toggleColor(c.name)} className={`w-7 h-7 rounded-full border-2 transition-all ${selectedColors.includes(c.name) ? "color-dot-active" : "border-gray-700 hover:border-gray-500"}`} style={{ backgroundColor: c.hex }} title={c.name} />
                ))}
              </div>
            </Section>
            <Section label="سایز" id="size">
              <div className="flex flex-wrap gap-1.5">
                {ALL_SIZES.map((sz) => (
                  <button key={sz} onClick={() => toggleSize(sz)} className={`min-w-[38px] h-9 text-[11px] font-medium border rounded-lg transition-all ${selectedSizes.includes(sz) ? "bg-[var(--accent)] text-white border-[var(--accent)]" : "bg-[var(--muted)] text-gray-400 border-[var(--border)] hover:border-[var(--accent)]"}`}>{sz}</button>
                ))}
              </div>
            </Section>
          ...
          <div className="filter-drawer-footer">
            <p className="text-xs text-gray-500">{filtered.length} محصول</p>
            <button onClick={onClose} className="btn-primary px-6 py-2.5 text-xs font-bold rounded-xl tracking-wider">
              دیدن نتایج
            </button>
          </div>
        </>
        ...
        <>
          ...
          <Section label="رنگ" id="color">
            <div className="flex flex-wrap gap-2">
              {ALL_COLORS.map((c) => (
                <button key={c.name} onClick={() => toggleColor(c.name)} className={`w-6 h-6 rounded-full border-2 transition-all ${selectedColors.includes(c.name) ? "color-dot-active" : "border-gray-700 hover:border-gray-500"}`} style={{ backgroundColor: c.hex }} title={c.name} />
              ))}
            </div>
          </Section>
          <Section label="سایز" id="size">
            <div className="flex flex-wrap gap-1.5">
              {ALL_SIZES.map((sz) => (
                <button key={sz} onClick={() => toggleSize(sz)} className={`min-w-[36px] h-8 text-[11px] font-medium border rounded-lg transition-all ${selectedSizes.includes(sz) ? "bg-[var(--accent)] text-white border-[var(--accent)]" : "bg-[var(--muted)] text-gray-400 border-[var(--border)] hover:border-[var(--accent)]"}`}>{sz}</button>
              ))}
            </div>
          </Section>
          <Section label="ویژه" id="special">
      ...
      <div className="overflow-hidden">
        ...
        <div className="hidden md:block max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="relative">
            <ScrollBtn scrollRef={categoryScrollRef} dir="prev" className="absolute left-0 top-1/2 -translate-y-1/2 z-10 w-8 h-8 flex items-center justify-center bg-black/60 hover:bg-black/80 text-white rounded-r-lg transition-all" />
            ...
            <ScrollBtn scrollRef={categoryScrollRef} dir="next" className="absolute right-0 top-1/2 -translate-y-1/2 z-10 w-8 h-8 flex items-center justify-center bg-black/60 hover:bg-black/80 text-white rounded-l-lg transition-all" />
          </div>
        </div>
      </div>
        ...
        <section id="featured" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-16 lg:py-20">
          ...
          <div className="overflow-hidden">
            ...
            <div className="relative hidden md:block">
              <ScrollBtn scrollRef={featuredScrollRef} dir="prev" className="absolute left-0 top-1/2 -translate-y-1/2 z-10 w-9 h-9 flex items-center justify-center bg-black/60 hover:bg-black/80 text-white rounded-r-lg transition-all" />
              ...
              <ScrollBtn scrollRef={featuredScrollRef} dir="next" className="absolute right-0 top-1/2 -translate-y-1/2 z-10 w-9 h-9 flex items-center justify-center bg-black/60 hover:bg-black/80 text-white rounded-l-lg transition-all" />
            </div>
          </div>
        </section>
      )}
      ...
      {newArrivals.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-16 lg:py-20 bg-[var(--muted)]/30">
          ...
          <div className="overflow-hidden">
            ...
            <div className="relative hidden md:block">
              <ScrollBtn scrollRef={newScrollRef} dir="prev" className="absolute left-0 top-1/2 -translate-y-1/2 z-10 w-9 h-9 flex items-center justify-center bg-black/60 hover:bg-black/80 text-white rounded-r-lg transition-all" />
              ...
              <ScrollBtn scrollRef={newScrollRef} dir="next" className="absolute right-0 top-1/2 -translate-y-1/2 z-10 w-9 h-9 flex items-center justify-center bg-black/60 hover:bg-black/80 text-white rounded-l-lg transition-all" />
            </div>
          </div>
        </section>
      )}
      ...
      {saleItems.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-16 lg:py-20">
          ...
          <div className="overflow-hidden">
            ...
            <div className="relative hidden md:block">
              <ScrollBtn scrollRef={saleScrollRef} dir="prev" className="absolute left-0 top-1/2 -translate-y-1/2 z-10 w-9 h-9 flex items-center justify-center bg-black/60 hover:bg-black/80 text-white rounded-r-lg transition-all" />
              ...
              <ScrollBtn scrollRef={saleScrollRef} dir="next" className="absolute right-0 top-1/2 -translate-y-1/2 z-10 w-9 h-9 flex items-center justify-center bg-black/60 hover:bg-black/80 text-white rounded-l-lg transition-all" />
            </div>
          </div>
        </section>
      )}
          ...
          <div className="flex-1 min-w-0">
            ...
            {filtered.length > 0 ? (
              <div className="overflow-hidden">
                ...
                {/* Tablet+ scroll */}
                <div className="relative hidden md:block">
                  <ScrollBtn scrollRef={allProductsScrollRef} dir="prev" className="absolute left-0 top-1/2 -translate-y-1/2 z-10 w-9 h-9 flex items-center justify-center bg-black/60 hover:bg-black/80 text-white rounded-r-lg transition-all" />
                  ...
                  <ScrollBtn scrollRef={allProductsScrollRef} dir="next" className="absolute right-0 top-1/2 -translate-y-1/2 z-10 w-9 h-9 flex items-center justify-center bg-black/60 hover:bg-black/80 text-white rounded-l-lg transition-all" />
                </div>
              </div>
            ) : (
