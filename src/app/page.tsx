...
export default function Home() {
  ...
  const filteredShoes = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      result = result.filter((s) => s.name.toLowerCase().includes(q) || s.namePersian.includes(q) || s.brand.toLowerCase().includes(q));
    }
    switch (sortBy) {
  ...
  return (
    <div>
      ...
      {featured.length > 0 && (
        <section id="featured" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 lg:py-20">
      ...
      {newArrivals.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 lg:py-20 bg-[var(--muted)]/30">
          <div className="flex items-center justify-between mb-6 sm:mb-10">
            <div>
      ...
      <section id="products" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 lg:py-20">
        ...
        {/* Filters - responsive */}
        <div className="flex flex-col lg:flex-row gap-3 sm:gap-4 mb-6 sm:mb-8 pb-4 sm:pb-6 border-b border-[var(--border)]">
          <div className="flex-1">
            <div className="relative">
              ...
              <input type="text" placeholder="جستجوی محصول..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
          ...
          <div className="flex gap-1.5 sm:gap-2 overflow-x-auto pb-1 sm:pb-0 scrollbar-none">
            <button onClick={() => setActiveCategory("all")}
              className={`whitespace-nowrap px-3 sm:px-4 py-1.5 sm:py-2 text-[11px] sm:text-xs font-medium transition-all rounded-xl ${activeCategory === "all" ? "filter-chip-active" : "bg-[var(--muted)] text-gray-400 hover:bg-[var(--border)]"}`}>
              همه
            </button>
            {categories.map((cat) => (
              <button key={cat.id} onClick={() => setActiveCategory(cat.id)}
                className={`whitespace-nowrap px-3 sm:px-4 py-1.5 sm:py-2 text-[11px] sm:text-xs font-medium transition-all rounded-xl ${activeCategory === cat.id ? "filter-chip-active" : "bg-[var(--muted)] text-gray-400 hover:bg-[var(--border)]"}`}>
                {cat.namePersian}
              </button>
            ))}
          </div>
          ...
          <select value={sortBy} onChange={(e) => setSortBy(e.target.value)}
            className="px-3 sm:px-4 py-2 sm:py-2.5 text-xs bg-[var(--muted)] border border-[var(--border)] focus:outline-none focus:border-[var(--accent)] transition-colors text-gray-400 rounded-xl">
